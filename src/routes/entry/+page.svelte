<script lang="ts">
  ...
  const updateEntry = async (e:SubmitEvent|MouseEvent) => {
    ...
    try {
      ...
      await dbFindByKeyAndUpdate('bookmark', targetEntry.key, targetEntry);
      resetEntry();
      editModal = false;
      errorMessage = '';
      refreshList();
  ...
  const onEdit = (key?:IDBValidKey) => async () => {
    if (key) {
      editModal = true;
      const result = await dbFindByKey('bookmark', key);
      targetEntry.key = key;
  ...
  <form action="#" on:submit={insertEntry} class="space-y-4">
    <Label class="space-y-2">
      ...
      <Input type="text" name="key" placeholder="key"
        ...
        class="dark:bg-nnrygray-400 text-white placeholder:text-gray-300"
      />
